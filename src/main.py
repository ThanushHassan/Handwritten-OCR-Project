...
def infer(model, fnImg):
    ...
    return recognized[0],correct_sentence(recognized[0])
...
def main():
    ...
    parser.add_argument(
        "--image", help="path to the image file for recognition", type=str, default=None)
    ...
    # Infer text on test image
        ...
        # Use custom image path if provided, otherwise use default
        imagePath = args.image if args.image else FilePaths.fnInfer
        if not os.path.exists(imagePath):
            print(f"Error: Image file not found at '{imagePath}'")
            print("Please provide a valid image path using: python main.py --image <path_to_image>")
            sys.exit(1)
...
def infer_by_web(path, option):
    ...
    model = Model(open(FilePaths.fnCharList).read(),
                  decoderType, mustRestore=False)
    recognized1,recognized2 = infer(model, path)
